...
### 4.3 Integrace se softwarem AutoHotkey
Pro maximální efektivitu a uživatelskou přívětivost využívám k obsluze button boxu program AutoHotkey (AHK). Tento software slouží jako most mezi hardwarem a operačním systémem Windows. Vzhledem k tomu, že firmware v mikrokontroléru odesílá specifické kombinace kláves CTRL + F13 až F19, které běžné aplikace ignorují, využívám AHK skript k zachycení těchto signálů a jejich následnému zpracování. V praxi to znamená, že stisk fyzického tlačítka vyvolá spuštění skriptu, který může otevřít libovolný program (např. Discord, Spotify nebo webový prohlížeč), spustit systémové makro nebo ovládat multimédia. Hlavní výhodou tohoto řešení je fakt, že pro změnu funkce tlačítek není nutné fyzicky zasahovat do zařízení ani přepisovat kód v CircuitPythonu; veškeré úpravy se provádí editací jednoduchého textového souboru v PC. Skript je nastaven tak, aby se automaticky spouštěl při startu systému, čímž je button box okamžitě připraven k použití.
